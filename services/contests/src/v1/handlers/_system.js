...
const _systemSubmissionUpdatedThusUpdateParticipantDetails = async (data, metadata) => {
    ...
    try {
        ...
        if (data._system.metadata.source === "submission-service") {
            ...
            if (!participant) {
                console.log("Sorry! This Participant with _id: ", created_by, " doesn't Exists....");

                data._system.metadata.success = false;
                data._system.metadata.message = `Sorry! This Participant with _id: ${created_by} doesn't Exists....`;
            ...
            data._system.metadata.message = `This Participant with _id: ${created_by} updated Successfully....`;
            data._system.metadata.updatedAt = (new Date()).toISOString();
        ...
        console.log(error);
        console.log("Something went wrong while handling in USER SERVICE while Updating the Participant's Details....");
        data._system.metadata.source = CURR_SERVICE_NAME;
        data._system.metadata.success = false;
