...
import { deleteFromCache, getFromCache, setToCache } from "../../../utils/v1/redisCacheManagement.js";
...
const _systemUpdateSubmissionDetailsOfPracticeProblemSubmission = async (data, metadata) => {
    ...
    try {
        ...
        if (data._system.metadata.source === "judge-service") {
            ...
            setToCache(`submissions:v1:_id:${_id}`, updateDataOfSubmission, 7200); // In Seconds thus it becomes 7200/60 = 120 Minutes = 2 Hours

            data = { ...data, result: updateDataOfSubmission };

            // Drop The System Only Data From the Data Before Sending it to "response" channel
            const updatedDataToSendToResponseChannel = await sanitizeResponse(data);
