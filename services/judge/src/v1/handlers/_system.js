    ...
    try {
        ...
        if (data._system.metadata.source === "problem-service") {
            ...
            const updatedTestCases = test_cases.map((testCase, index) => {
                return {
                    ...testCase,
                    token: submissionResponseTokens[index],
                };
            })
...
const _systemGetUpdatesOfTheSubmissionOfPracticeProblem = async (data, metadata) => {
    ...
    try {
        ...
        if (data._system.metadata.source === "judge-service") {
            ...
            // Make Request to the Code Execution Engine 
            const response = await axios.get(`${CODE_EXECUTION_ENGINE_API_URL}/submissions/batch?token=${allTokenString}`);

            const allTokenStringResponseStatuses = response.data;
            /*
             [ 
                {
                    ..., 
            ...
            */
            ...
            const isAnyTestCaseRunningIsPending = allTokenStringResponseStatuses.find((tokenResponseStatus) => tokenResponseStatus.status_id <= 2);
